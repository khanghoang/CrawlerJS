...
var getResult = function(font,url,data,stream){
  ...
  if(font.preview == 0){
    for(var i in font.extractors){
      if(typeof font.extractors[i].csv != 'undefined'){
        streamLog[i]['csv'].write(toCsv(JSON.stringify([data]),font.extractors[i].csv.separator,font.extractors[i].csv.delimiter));
      }
      if(typeof font.extractors[i].json != 'undefined'){
        streamLog[i]['json'].write(JSON.stringify(data)+'\n');
      }
      if(typeof font.extractors[i].mongoCollection != 'undefined'){
        streamLog[i]['mongoCollection'].insert(data);
      }
    }
  }else{
    console.log(data);
