readMongoDb = function(font,url,config){
  ...
  updateTerm = function(font,url,config,doc){
    if(font.preview == 0){
      var data = {$set:{cjsStatus:getUuid('xxxxxxxxxxxxxxxx')}};
    }else{
      var data = {$set:{}};
    }
    dbUrls.collection(font[font['type']+'Attributes'][font[font['type']+'CurrentEntry']]['collection']).update({_id:doc._id},data,{upsert:true,safe:false},
      function(err,data){
        if(err){
          console.log(err);
        }else{
