    ...
    if(font.preview == 0){
      ...
      for(var i in font.extractors){
        ...
        if(typeof font.extractors[i].csv != 'undefined'){
          stream[i]['csv'] = fs.createWriteStream(font.extractors[i].csv.name, {flags: 'w'});
          streamLog[i]['csv'] = fs.createWriteStream(font.extractors[i].csv.name.replace(".","_log."), {flags: 'w'});
