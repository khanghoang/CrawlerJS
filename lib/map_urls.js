CrawlerJS = function(font,config){
  ...
  if(font.preview == 0){
    stream = createStream(font,'-results.',config);
    streamLog = createStream(font,'-log.',config);
  }else{
    stream = '';
    streamLog = '';
  }
  if(font.preview < 2){
    font.getAttributes = getAttributes(font.get,config);
    font.postAttributes = getAttributes(font.post,config);
...
getAttributes = function(entry,config){
